...
public class TestCreateGameController extends TestCase {
	...
	public void testSimple() {
		...
		String xmlString = Message.requestHeader()
				+ "<createGameRequest name='test'/></request>";
		Message request = new Message(xmlString);

		// get response after processing this request
		Message response = new CreateGameRequestController(model).process(
				client1, request);
		...
		xmlString = Message.requestHeader()
				+ "<joinGameRequest gameId='hg12jhd' name='other'/></request>";
		request = new Message(xmlString);

		// get response after processing this request
		Message joinClient2Response = new JoinGameRequestController(model)
				.process(client2, request);
