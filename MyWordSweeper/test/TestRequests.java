...
public class TestRequests extends TestCase {
	...
	public void testCreateGameRequest() {
		Message m = new Message(requestHeader
				+ "<createGameRequest name='george'/>" + "</request>");
	 ...
	 * 
	 * Board of each player is represented with the player. That is, for now not
	 * evern the total size of the board is encoded to the client.
	 * 
	 * Two players are on the board, and multiplier is at (4,3). Note that
	 * player2 is managing user.
	 */
	...
	public void testLockGameRequest() {
		Message m = new Message(requestHeader
				+ "<lockGameRequest gameId='specialGame'/>" + "</request>");
		System.out.println(m.toString());
	}
	...
	public void testLockGameResponse() {
		Message m = new Message(responseHeader
				+ "<lockGameResponse gameId='specialGame'/>" + "</response>");
		System.out.println(m.toString());
	}
