 ...
 * from clients. You should follow this template when designing YOUR protocol
 * handler.
 * <p>
 * To avoid issues with multiple clients submitting requests concurrently,
 * notice that the {@link #process(ClientState, Message)} method is
 * synchronized. This will ensure that no more than one server thread executes
 * this method at a time.
 * <p>
 * Also extended to support detection of client disconnects so these can release
 * the lock if indeed the client was the one locking the model.
 */
