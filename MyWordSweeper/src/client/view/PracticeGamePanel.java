...
public class PracticeGamePanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		btnReturnButton.setBounds(height / 180, width / 320, height / 9,
				width / 64);
		add(btnReturnButton);

		ReturnMenuPanelController returnMenuController = new ReturnMenuPanelController(
				model, application);
		btnReturnButton.addMouseListener(returnMenuController);

		JButton btnResetGame = new JButton("Reset Game");
		btnResetGame.setFont(new Font("Tahoma", Font.PLAIN, height / 50));
		btnResetGame.setBounds(23 * height / 45, 17 * width / 640,
				3 * height / 20, width / 64);
		add(btnResetGame);
		ResetGameInPracticeController resetGameController = new ResetGameInPracticeController(
				model, application);
